...
> **Codename:** CAIRN (*to bear witness; to certify as true*).
> Intellectual lineage, for your own amusement: a **critical apparatus** — the scholarly annotation layer that shows every reading's source and variants. The public-facing name stays CAIRN because the whole point of this project is legibility to a non-specialist buyer. Swap freely.
...
**v1:** CAIRN ships as a set of **deterministic tools** (an MCP server + CLI) that **Claude Code invokes during a session**. The reasoner *is* the Claude Code agent. CAIRN makes **no model calls of its own** — the only model in the loop at runtime is the agent driving the tools. (The eval harness is the one place a model-as-judge appears, isolated there.)

**v2 (later):** an API-wrapped service where CAIRN sits *between* a model and the user. Deferred.

This pivot relocates the grounding guarantee, and the relocation is the most important thing in this brief:

- In the v2/API design CAIRN **wraps** the model, so it can structurally intercept output and refuse anything ungrounded.
- As a Claude Code tool the agent sits **above** CAIRN and calls it, so CAIRN **cannot intercept** the agent's free text. The guarantee instead comes from (a) **deterministic tools** the agent calls and (b) a **mandatory `verify` + `log` step** the agent is bound to call before presenting an answer.

The honest boundary that follows — split along the determinism line:

| | Deterministic — CAIRN owns it (the real v1 guarantee) | A model judgment — NOT enforced at runtime in v1 |
|---|---|---|
...
> refresh-and-update** (`CAIRN_Patent_Tailoring_Consideration.md`). EDGAR stays the reference build.
...
- **I6 — Deterministic evidence layer.** Given the same corpus + query, retrieval and span-mapping are reproducible (seeded). In v1 CAIRN makes **no model calls at all** at runtime — the reasoner is the Claude Code agent, so every CAIRN tool is a pure deterministic function and the oracle's component tests are stable by construction.
...
3. **Answer verification + abstention (deterministic)** — the agent composes; CAIRN provides a `verify` tool that resolves every cited span and a deterministic abstention *trigger*. CAIRN does not compose prose in v1.
...
**`cairn_rig.py`** (single script / notebook):
...
These test CAIRN's pure-function tools directly, no agent in the loop:
...
Run the actual Claude Code agent + CAIRN tools over the golden set in print/headless mode and score the transcript:
...
In v1 the **Claude Code agent composes**; CAIRN provides the deterministic machinery and the verify-and-log step that re-imposes the guarantee (see Runtime model).
...
| `get_document(doc_id)` | Full hash-verified canonical text — the agent reads freely (D11) | yes | none (read) |
...
`check_claim` is worth highlighting separately in the portfolio — "paste a sentence, find out if your own documents actually back it" is an instantly graspable client demo. A project `CLAUDE.md` documents the required loop (search / read → compose from the corpus, grounding every sentence in a verified span → `check_support` → `verify` → present-or-abstain) so the agent follows it every session. The agent **reads freely** (D11) — `search_corpus` and `get_document` feed its reasoning; the constraint is on output (every claim binds to a verified span), not on what it may read.

Both `verify` and `check_claim` share one deterministic primitive — an **atom resolver** (ROADMAP **D9**): the agent decomposes its answer into load-bearing atoms (numbers, dates, named entities), binds each to a specific `(doc_id, content_hash, char_start, char_end)`, and the resolver confirms the slice at that offset equals the atom *exactly* and hash-matches (I3). The binding is **offset-specific** (this occurrence, not "the literal exists somewhere") and its relevance is judged offline (Layer-E) — that, not an input restriction, is what "no whole-corpus fishing" means under D11. The agent **parameterizes** the check (supplies atoms + bindings); it never **authors** it. `verify` independently re-extracts atoms from the final answer so an untagged figure can't slip through. This kills *invented* citations; it does **not** confirm entailment (existence ≠ support — measured offline at Layer-E, structural in v2). Open implementation contingencies live under ROADMAP M2-T1.
...
- **Language:** Python for CAIRN tools (MCP server + CLI) and the rig; TypeScript/React for the demo.
- **Runtime:** Claude Code is the reasoner and the host. CAIRN ships as an **MCP server + CLI** with **no model calls of its own in v1** (I6). The only model-as-judge lives in the Layer-E eval harness and can be a scripted **headless Claude Code** invocation, isolated there.

...
2. **Abstain when unsupported (two mechanisms, D12):**
   ...
   - **Four outcomes, not present-vs-silent (D16):** *answer* · *abstain* (stay silent) · **correction** — when the question rests on a **false premise**, don't silently abstain: present a **grounded refutation** that cites the contradicting span (e.g. "assets did not decline — they rose, see ⟨span⟩") · **partial** — answer the in-corpus part and **explicitly flag** what's out of corpus. `correction` and `partial` still *present* and still go through `verify`.
...
5. **`verify(answer)` before presenting.** It confirms every atom resolves at its offset + hash-matches (I1/I3), flags unbound figures, and recomputes derived values. If `not ok` → fix the binding or abstain. (`verify` confirms a citation is *real*, not that it *entails* — entailment is Layer-E.) Pass `outcome` (D16: `answer` / `correction` / `partial`) so the audit log and evidence view record *which kind* of presentation it was (a grounded correction renders distinctly).
